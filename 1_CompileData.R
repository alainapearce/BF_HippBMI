...
for(p in 1:length(QC_parIDs)){
  ...
  if(nrow(QC_pardat_BF) > 1){
    ...
    if(length(nMax)>1){
      ...
      compare_dat$totalScore = rowSums(compare_dat[c('minIQR', 'minRinging', 'minGhosting', 'minBlurriness', 'minRinging_dif', 'minGhosting_dif', 'minBlurriness')])
    }
    ...
    if(length(nMax)>1){
      ...
      compare_dat$totalScore = rowSums(compare_dat[c('minIQR', 'minRinging', 'minGhosting', 'minBlurriness', 'minRinging_dif', 'minGhosting_dif', 'minBlurriness', 'date')])
...
TPMall_QC$cAge_yr = round(TPMall_QC$cAge_yr, 1)
write.csv(TPMall_QC[TPMall_QC$Keep == 'Y', !grepl('cAge_mo', names(TPMall_QC))], 'Data/BFstructural_TPMall_QCdata.csv', row.names = FALSE)

write.csv(TPMall_QC[TPMall_QC$Keep == 'Y', c('LabID', 'parID')], 'Data/BFstructural_parlist.csv', row.names = F)

BFstructural_covars$cAge_yr = round(BFstructural_covars$cAge_yr, 1)
write.csv(BFstructural_covars[!grepl('cAge_mo', names(BFstructural_covars))], 'Data/BFstructural_covars.csv', row.names = F)

BF_dat$cAge_yr = round(BF_dat$cAge_yr, 1)
write.csv(BF_dat[BF_dat$Keep == 'Y', !grepl('cAge_mo', names(BF_dat))], 'Data/BFstructural_Alldata.csv', row.names = FALSE)
